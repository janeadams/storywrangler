...
function showloadingpanel(){
    ...
    d3.select('.pageOverlay').css({ 'display': 'block' }).append('div').attr('class','loader')
}

function hideloadingpanel(){
    //console.log('Hiding loading panel...')
    d3.select('.pageOverlay').css({ 'display': 'none' })
...
function showAlert(msg){
    ...
    //hideAlert()
    d3.select('.pageOverlay').css({ 'display': 'block' }).append('div').attr('class','alert').html('<p>'+msg+'</p>')
}
