function addGlyphs(chart){
    ...
    Object.keys(ngramData).forEach(rt_state => {
        ...
        chart.selectorPlot.append('path')
            .datum(ndata.filter(selectorline.defined()))
    ...
    Object.keys(ngramData).forEach(rt_state => {
        ...
        function handleMouseOver(d) {
            d3.select(this).style("r", 7).style("fill",darkColor)
            div.style('border-color', darkColor)
            ...
            div.html(`<span style="font-weight:bold; color:${darkColor};">${Ngram}</span><br/><span style="font-weight:bold;">Date:</span> ${dateFormatter(d[0])}<br/><span style="font-weight:bold;">${sentenceCase(params['metric'])}:</span> ${formattedValue}<br/><span style="font-style:italic;">${RTlabel}</span>`)
                .style("left", (d3.event.pageX + 5) + "px")
                .style("top", (d3.event.pageY - 28) + "px");
        }
        function handleMouseOut() {
            d3.select(this).style("r", 2).style("fill",mainColor)
            div.transition()
                .duration(0)
                .style("opacity", 0);
            div.style("left", "0px")
