...
class Query:
    ...
    def prepare_day_query(self, date=None, max_rank=None, rt=True, min_count=None):
        if max_rank:
            if rt:
                return {"time": date if date else self.last_updated, "rank": {"$lte": max_rank}}
            else:
                return {"time": date if date else self.last_updated, "rank_no_rt": {"$lte": max_rank}}
        elif min_count:
            if rt:
                return {"time": date if date else self.last_updated, "count": {"$gte": min_count}}
            else:
                return {"time": date if date else self.last_updated, "count_no_rt": {"$gte": min_count}}
        else:
    ...
    def prepare_divergence_query(self, date=None, max_rank=None, rt=True):
        if max_rank:
            if rt:
                return {"time_2": date if date else self.last_updated, "rank_change": {"$lte": max_rank, "$gte": -(max_rank)}}
            else:
                return {"time_2": date if date else self.last_updated, "rank_change_noRT": {"$lte": max_rank, "$gte": -(max_rank)}}
        else:
