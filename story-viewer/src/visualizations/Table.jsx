...
function EnhancedTableHead(props) {
    ...
    const headCells = makeHeaders(props.options)
...
const EnhancedTable = ( props ) => {
    ...
    let options = metricOptions(props.viewer,1)
    const columnOptions = () => {
        let mapping = {}
        Object.entries(options).forEach(([key, value]) => {
            let keyFormatted = props.params.rt ? key : key+"_no_rt"
            let valueFormatted = props.params.rt ? value : value + " (No Retweets)"
            mapping[keyFormatted] = valueFormatted})
        //console.log({mapping})
        return mapping
    }
    const rows = makeRows(props.data, options)
    //console.log({rows})
    ...
    return (
        <div className={classes.root}>
            <Paper className={classes.paper}>
                <EnhancedTableToolbar numSelected={selected.length} title={pageMeta(props.viewer).title} />
                <TableContainer>
                    <Table
                        ...
                        <TableBody>
                            ...
                            {emptyRows > 0 && (
                                <TableRow style={{ height: (dense ? 33 : 53) * emptyRows }}>
                                    <TableCell colSpan={Object.keys(options).length} />
                                </TableRow>
                            )}
                        </TableBody>
                    </Table>
