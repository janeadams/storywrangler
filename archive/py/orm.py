...
def send_list():
    ...
    threshold = request.args.get('thresh')
    # Default is 500
    ...
    if language is None:
        language = 'en'
    # Pull the wordcount from the URL params, e.g. 1,2,3
    ngram = request.args.get('words')
    # By default, return top 1grams
    if ngram is None:
        ngram = 1
    else:
        ngram = int(ngram)
    print_info({"wordcount":ngram,"minrank":threshold,"metric":metric,"language":language})
    # Select the location based on the wordcount (1grams, 2grams, 3grams, etc.), by counting spaces
    db = client[str(ngram)+'grams']
    #wordlist = list(db[language].distinct('word',{metric:{'$lte':threshold}}))
    wordlist = list(db[language].aggregate( [ { "$group" : { "_id" : "$word" , "minRank": {"$min":"$rank"}} } ], allowDiskUse=True))
