function drawAllTimeseries() {
    ...
    var fullXrange = d3.extent(new Date(2008, 1, 1), new Date())
    console.log('fullXrange =', fullXrange)
    var fullYrange = [0, 100000]
    console.log('fullYrange =', fullYrange)

    // maximum date range allowed to display
    var mindate = fullXrange[0]
    var maxdate = fullXrange[1]

    for (var i = 0; i < querydata.length; i++) {
        if (i = 0) {
            // the date range of available data:
            fullXrange = d3.extent(querydata[i]['dates'])
            fullYrange = [0, d3.max(querydata[i][params['metric']])]
        } else {
            function updateSizing(data) {
                var thisXrange = d3.extent(data["dates"])
                if (thisXrange[0] < fullXrange[0]) {
                    fullXrange[0] = thisXrange[0]
                    console.log('because of ', data["word"], 'updated fullXrange[0] (min) =', fullXrange[0])
                }
                if (thisXrange[1] > fullXrange[0]) {
                    fullXrange[0] = thisXrange[0]
                    console.log('because of ', data["word"], 'updated fullXrange[1] (max) =', fullXrange[1])
                }
                var thisYrange = [0, d3.max(data[params['metric']])]
                if (thisYrange[1] > fullYrange[1]) {
                    fullYrange[1] = thisYrange[1]
                    console.log('because of ', data["word"], 'updated fullYrange[1] (max) =', fullYrange[1])
                }
            }
            updateSizing(querydata[i])
        }
    }
...
function drawEachTimeseries(data) {
    ...
    // Create a list of all dates in our date range, including the ones missing from our set
    var dates_m = d3.scaleTime().domain(dates).ticks(d3.timeDay, 1)
    console.log('There are ', dates_m.length - dates.length, ' dates missing from the range')
    // Create a list of value/date pairs for all dates in our date range, including dates missing from our data set
    var pairs_m = []
    // For each value in our list of value/date pairs...
    for (var i = 0; i < dates_m.length; i++) {
        // See if it's in our list of dates that we have
        var n = dates.indexOf(dates_m[i].getTime());
        // If we have the date in our dataset, fill with that value
        if (n > -1) {
            var val = values[n];
            // If we don't have that date in our dataset, fill with 'null'
        } else {
            var val = null;
        }
        // Add all value/date pairs to our array of objects
        pairs_m.push({ value: val, date: dates_m[i] });
    }
    // Create another value/date pair list without missing values
    var pairs = pairs_m.filter(function(d) { return d.value !== null; });
