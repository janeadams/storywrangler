...
def give_instructions():
    return "Enter a URL containing a 1, 2, or 3-word query</br>in the format <b>/api/</b><em>&lt;query&gt;</em><b>?lang=</b><em>&lt;en,es,ru&gt;</em><b>&metric=[rank,counts,freq]</b></br>e.g. <a href='https://storywrangling.org/api/happy birthday?lang=en&metric=[counts]'>https://storywrangling.org/api/happy birthday?lang=en&metric=[counts]</a></br></br>Note: Emojis are supported! 🐙</br><a href='https://storywrangling.org/api/🐙?metric=[rank]'>https://storywrangling.org/api/🐙?metric=[rank]</a>"
...
def simple_response():
    ...
    with open('prod/api/querylog.csv','a') as fd:
    ...
    with open('prod/api/responselog.csv','a') as fd:
        write_outfile = csv.writer(fd)
...
def get_data(query):
    ...
    # querylog columns = ['pid','query','src','wordcount','lang','metrics','ip','sent','date']
    with open('prod/api/querylog.csv','a') as fd:
        write_outfile = csv.writer(fd)
        write_outfile.writerow([int(pid),str(query),str(src),int(ngram),str(language),metric,str(ip),sent,str(start)])
        fd.close()
    output=dict()
    output['word']=query
