function sendQuery(formatted_query, APIsource){
    let urls = []
    showloadingpanel()
    urls.push(encodeURI(`${APIsource}/api/ngrams/${formatted_query}?src=ui&language=${params["language"]}&metric=${params['metric']}&rt=false`))
    urls.push(encodeURI(`${APIsource}/api/ngrams/${formatted_query}?src=ui&language=${params["language"]}&metric=${params['metric']}&rt=true`))
    ...
    d3.json(url).then((data, error) => {
        ...
        if (data['errors'].length > 0){
            ...
            if (returnedNgrams.length > 0) {
                ...
                resetPage()
                document.getElementById("compare-link").setAttribute("href", `index.html?ngrams=${Ngram}`)
            }
        }
    })
}
