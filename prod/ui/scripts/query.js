...
function filterSubmission() {
    ...
    params['noRT']=d3.select("input[value ='noRT']").property('checked');
    console.log("params['noRT'] = ",params['noRT']);
    updateURL();
}
...
function updateURL() {
    ...
    */
    for (var p of ['queries', 'metric', 'lang', 'scale','noRT']) {
        console.log("var p = ", p);
        console.log("params[p] = ", params[p], " defaultparams[p] = ", defaultparams[p]);
        if (params[p] !== defaultparams[p]) {
            customparams[p] = params[p]
